 ...
 */
uint32_t find_got_entry_address(const char *module_path,
		const char *symbol_name) {
	uint32_t module_base = get_module_base(-1, module_path);
	int find = 0;

	if (module_base == 0) {
		LOGE("[-] it seems that process %d does not dependent on %s", getpid(),
				module_path);
		return 0;
	}
	...
	if (read(fd, elf_header, sizeof(Elf32_Ehdr)) != sizeof(Elf32_Ehdr)) {
		LOGE("[-] read %s error! in %s at line %d", module_path, __FILE__,
				__LINE__);
		return 0;
	}
	...
	for (; i < elf_header->e_shnum; i++) {
		...
		if (read(fd, shdr, sizeof(Elf32_Shdr)) != sizeof(Elf32_Shdr)) {
			LOGE("[-] read %s error! i = %d, in %s at line %d", module_path, i,
					__FILE__, __LINE__);
			return 0;
		}
	}
	...
	for (i = 0; i < relplt_shdr->sh_size / sizeof(Elf32_Rel); i++) {
		...
		if (strcmp(dynstr + dynsymtab[ndx].st_name, symbol_name) == 0) {
			LOGI("[+] got entry offset of %s: 0x%x", symbol_name,
					rel_ent->r_offset);
			find = 1;
			break;
		}
 ...
 * return 0 if FAILED
 */
uint32_t do_hook(const char *module_path, uint32_t hook_func,
		const char *symbol_name) {
	uint32_t entry_addr = find_got_entry_address(module_path, symbol_name);

	if (entry_addr == 0) {
		LOGE("[+] can not found %s address in %s", symbol_name, module_path);
		return 0;
	}

	LOGD("[+] found symbol %s addr=0x%x", symbol_name, entry_addr);
