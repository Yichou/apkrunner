...
//      put("touchnavigation", InputDevice.SOURCE_TOUCH_NAVIGATION);
    ...
    public void sendKeyEvent2(int keyCode) {
    	int inputSource = InputDevice.SOURCE_KEYBOARD;
    	boolean longpress = false;
    	long now = SystemClock.uptimeMillis();
        injectKeyEvent2(new KeyEvent(now, now, KeyEvent.ACTION_DOWN, keyCode, 0, 0,
                KeyCharacterMap.VIRTUAL_KEYBOARD, 0, 0, inputSource));
        if (longpress) {
            injectKeyEvent2(new KeyEvent(now, now, KeyEvent.ACTION_DOWN, keyCode, 1, 0,
                    KeyCharacterMap.VIRTUAL_KEYBOARD, 0, KeyEvent.FLAG_LONG_PRESS,
                    inputSource));
        }
        injectKeyEvent2(new KeyEvent(now, now, KeyEvent.ACTION_UP, keyCode, 0, 0,
                KeyCharacterMap.VIRTUAL_KEYBOARD, 0, 0, inputSource));
    }
    
    protected void injectKeyEvent2(KeyEvent event) {
    	try {
    ...
    public void sendTap(float x, float y) {
    	sendTap(InputDevice.SOURCE_TOUCHSCREEN, x, y);
    }
    ...
    public void sendSwipe(float x1, float y1, float x2, float y2, int duration) {
    	sendSwipe(InputDevice.SOURCE_TOUCHSCREEN, x1, y1, x2, y2, duration);
    }
    ...
    protected abstract void injectKeyEvent(KeyEvent event);
...
//      Log.i(TAG, "injectMotionEvent: " + event);
    ...
    protected abstract void injectMotionEvent(MotionEvent event);
    ...
    public static BaseInput getInstance() {

    	try {
			Class.forName("android.hardware.input.InputManager");
			return new InputImpl4();
		} catch (Exception e) {
		}
//		
		return new InputImpl23();
    }
}
